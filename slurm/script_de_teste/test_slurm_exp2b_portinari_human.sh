#!/bin/bash
#SBATCH --job-name=test_exp2b_port
...
notify() {
    ...
    if [ $code -eq 0 ]; then
        ...
        python3 scripts/manda_email.py \
            "✅ [TESTE] exp2b_portinari_human CONCLUÍDO — Phocus4" \
            "Job de teste finalizado com sucesso. Resultados em: $ROOT/outputs/test_exp2b_portinari_human/"
    else
        source "$VENV/bin/activate"
        python3 scripts/manda_email.py \
            "❌ [TESTE] exp2b_portinari_human FALHOU — Phocus4" \
            "Job de teste abortou com erro (código $code). Verifique o log: $ROOT/slurm-${SLURM_JOB_ID}.out"
    fi
}
trap notify EXIT

echo "--- Fase 1: sampling + generation (Janus, sem captioning) [MODO TESTE: 5 amostras] ---"
source "$VENV/bin/activate"
python3 run.py --config configs/exp2b_portinari_human.yaml --steps sampling,generation --test \
    || { echo "ERRO fase Janus"; exit 1; }
deactivate

echo "--- Fase 2: scoring (ArtCLIP) ---"
source "$VENV_APDDV2/bin/activate"
python3 run.py --config configs/exp2b_portinari_human.yaml --steps scoring --test \
