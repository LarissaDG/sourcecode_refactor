#!/bin/bash
#SBATCH --job-name=test_exp4_noise
...
notify() {
    ...
    if [ $code -eq 0 ]; then
        ...
        python3 scripts/manda_email.py \
            "✅ [TESTE] exp4_noise CONCLUÍDO — Phocus4" \
            "Job de teste finalizado com sucesso. Resultados em: $ROOT/outputs/test_exp4_noise/"
    else
        source "$VENV/bin/activate"
        python3 scripts/manda_email.py \
            "❌ [TESTE] exp4_noise FALHOU — Phocus4" \
...
python3 run.py --config configs/exp4_noise.yaml --steps sampling,scoring --test \
    || { echo "ERRO"; exit 1; }
deactivate
