...
notify() {
    ...
    if [ $code -eq 0 ]; then
        ...
        python3 scripts/manda_email.py \
            "✅ [TESTE] exp2a_portinari CONCLUÍDO — Phocus4" \
            "Job de teste finalizado com sucesso. Resultados em: $ROOT/outputs/test_exp2a_portinari/"
    else
        source "$VENV/bin/activate"
        python3 scripts/manda_email.py \
            "❌ [TESTE] exp2a_portinari FALHOU — Phocus4" \
...
source "$VENV/bin/activate"
python3 run.py --config configs/exp2a_portinari.yaml --steps sampling,captioning,generation --test \
    || { echo "ERRO fase Janus"; exit 1; }
deactivate

echo "--- Fase 2: scoring (ArtCLIP) ---"
source "$VENV_APDDV2/bin/activate"
python3 run.py --config configs/exp2a_portinari.yaml --steps scoring --test \
