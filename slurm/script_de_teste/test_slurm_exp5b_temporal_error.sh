#!/bin/bash
#SBATCH --job-name=test_exp5b_temp
...
notify() {
    ...
    if [ $code -eq 0 ]; then
        ...
        python3 scripts/manda_email.py \
            "✅ [TESTE] exp5b_temporal_error CONCLUÍDO — Phocus4" \
            "Job de teste finalizado com sucesso. Resultados em: $ROOT/outputs/test_exp5b_temporal_error/"
    else
        source "$VENV/bin/activate"
        python3 scripts/manda_email.py \
            "❌ [TESTE] exp5b_temporal_error FALHOU — Phocus4" \
...
python3 run.py --config configs/exp5b_temporal_error.yaml --steps sampling,scoring --test \
    || { echo "ERRO"; exit 1; }
deactivate
