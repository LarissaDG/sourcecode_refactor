...
notify() {
    ...
    if [ $code -eq 0 ]; then
        ...
        python3 scripts/manda_email.py \
            ...
            "Job finalizado com sucesso. Resultados em: /snfs1/speed/larissa.gomide/outputs/exp1_apdd/"
    else
        source "$VENV/bin/activate"
        python3 scripts/manda_email.py \
...
source "$VENV/bin/activate"
python3 run.py --config configs/exp1_apdd.yaml --steps sampling,captioning,generation,samples \
