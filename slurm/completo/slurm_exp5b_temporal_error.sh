...
notify() {
    ...
    if [ $code -eq 0 ]; then
        ...
        python3 scripts/manda_email.py \
            "✅ exp5b_temporal_error CONCLUÍDO — Phocus4" \
            "Job finalizado com sucesso. Resultados em: /snfs1/speed/larissa.gomide/outputs/exp5b_temporal_error/"
    else
        source "$VENV/bin/activate"
        python3 scripts/manda_email.py \
            "❌ exp5b_temporal_error FALHOU — Phocus4" \
...
python3 run.py --config configs/exp5b_temporal_error.yaml --steps sampling,samples,scoring \
    || { echo "ERRO"; exit 1; }
deactivate
